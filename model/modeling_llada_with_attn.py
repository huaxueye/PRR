...
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor, q_full: torch.Tensor,
                block_end_index: Optional[torch.Tensor] = None) -> Tuple[torch.Tensor, torch.Tensor]:
        if self.config.rope_full_precision:
            q_, k_, q_full_ = q.float(), k.float(), q_full.float()
        else:
            q_, k_, q_full_ = q, k, q_full
        ...
        with torch.autocast(q.device.type, enabled=False):
            ...
            q_full_ = self.apply_rotary_pos_emb(pos_sin, pos_cos, q_full_)

        return q_.type_as(q), k_.type_as(k), q_full_.type_as(q)
...
class LLaDABlock(nn.Module):
    ...
    def attention(
        ...
        layer_past: Optional[Tuple[torch.Tensor, torch.Tensor, torch.Tensor]] = None,
        use_cache: bool = False,
        replace_position: Optional[torch.Tensor] = None,
        output_attentions: bool = False,
        output_attentions_targets: Optional[Dict[str, List[int]]] = None,
        ...
        past_key, past_value, past_query = None, None, None
        if layer_past is not None:
            past_key, past_value, past_query = layer_past
            if replace_position is None:
                k = torch.cat((past_key, k), dim=-2)
                v = torch.cat((past_value, v), dim=-2)
                q_full = torch.cat((past_query, q), dim=-2)
            else:
                ...
                for batch_idx in range(B):
                    ...
                    if len(batch_replace_indices) > 0:
                        ...
                        past_query[batch_idx, :, batch_replace_indices] = q[batch_idx, :, :len(batch_replace_indices)]
        ...
        if output_attentions and output_attentions_targets is not None:
            target_layers = output_attentions_targets.get('layers', [])
            if self.layer_id in target_layers:
                # print("Computing attention matrix for layer", self.layer_id)
                should_compute_matrix = True
                target_heads = output_attentions_targets.get('heads', None)

        attn_weights = None

        if should_compute_matrix:
            # print("Computing full attention matrix for layer", self.layer_id)
            scale = 1 / math.sqrt(q_full.size(-1))
            # Q_full * K_full^T -> (B, H, Total_Len, Total_Len)
            scores = torch.matmul(q_full, k.transpose(2, 3)) * scale
            attn_weights_full = F.softmax(scores, dim=-1, dtype=torch.float32).to(q.dtype)

            if target_heads is not None and len(target_heads) > 0:
                attn_weights = attn_weights_full[:, target_heads, :, :]
            else:
                attn_weights = attn_weights_full
        # =========================================================================

        present = (k, v, q_full) if use_cache else None # present: None
        query_len, key_len = q.shape[-2], k.shape[-2] # could be different if layer_past not None

        if self.config.rope:
            # Apply rotary embeddings.
            if replace_position is None:
                q, k, q_full = self.rotary_emb(q, k, q_full)
            else:
                ...
                q, k, q_full = self.rotary_emb(q, k, q_full, max_replace_pos)
    ...
    def forward(
        ...
        if self._activation_checkpoint_fn is not None:
            att, cache, attn_weights = self._activation_checkpoint_fn(  # type: ignore
                ...
                output_attentions=output_attentions, output_attentions_targets=output_attentions_targets
            )
        else:
            att, cache, attn_weights = self.attention(q, k, v, attention_bias, layer_past=layer_past, use_cache=use_cache, 
                                                          output_attentions=output_attentions, output_attentions_targets=output_attentions_targets
            )
...
class LLaDALlamaBlock(LLaDABlock):
    ...
    def forward(
        ...
        if self._activation_checkpoint_fn is not None:
            ...
            att, cache, attn_weights = self.attention(q, k, v, attention_bias, layer_past=layer_past, use_cache=use_cache,replace_position=replace_position,
                                                      output_attentions=output_attentions, output_attentions_targets=output_attentions_targets)
...
class LLaDAOutput(NamedTuple):
    ...
    attn_key_values: Optional[List[Tuple[torch.Tensor, torch.Tensor, torch.Tensor]]]
    """
    ...
    """
    Attentions weights from each block.
    """
...
class LLaDABlockGroup(nn.ModuleList):
    ...
    def forward(
        ...
        x: torch.Tensor,
        attention_bias: Optional[torch.FloatTensor] = None,
        layers_past: Optional[List[Tuple[torch.Tensor, torch.Tensor, torch.Tensor]]] = None,
        use_cache: bool = False,
        output_attentions: bool = False,
        output_attentions_targets: Optional[Dict[str, List[int]]] = None,
    ) -> Tuple[torch.Tensor, Optional[List[Tuple[torch.Tensor, torch.Tensor]]], Optional[Tuple[torch.Tensor]]]:
        ...
        for block_idx, block in enumerate(self):
            ...
            kwargs = {
                ...
                "output_attentions_targets": output_attentions_targets,
            }
            ...
            if (
                ...
                # shape: (batch_size, seq_len, d_model)
                x, cache, attn_weights = block(x, attention_bias=attention_bias, layer_past=layer_past, use_cache=use_cache, 
                                               output_attentions=output_attentions,
                output_attentions_targets=output_attentions_targets,
            )
...
class LLaDAModel(nn.Module):
    ...
    def forward(
        ...
        if self.config.block_group_size == 1:
            for block_idx, block in enumerate(self.transformer.blocks):
                ...
                if (
                    ...
                    # shape: (batch_size, seq_len, d_model)
                    x, cache, attn_weights = block(x, attention_bias=attention_bias, layer_past=layer_past, use_cache=use_cache,replace_position=replace_position,
                                                   output_attentions=output_attentions, output_attentions_targets=output_attentions_targets)
                if attn_key_values is not None:
                    assert cache is not None
                    attn_key_values.append(cache)
            ...
            for group_idx, block_group in enumerate(self.transformer.block_groups):
                ...
                x, cache, group_attentions = block_group(
                    x, attention_bias=attention_bias, layers_past=layers_past, use_cache=use_cache,output_attentions=output_attentions
                )
                if attn_key_values is not None:
                    assert cache is not None
                ...
                if all_attentions is not None:
                    assert group_attentions is not None
                    all_attentions = all_attentions + group_attentions
        ...
        return LLaDAOutput(logits=logits, attn_key_values=attn_key_values, 
                           ...
                           attentions=all_attentions if output_attentions else None)  # type: ignore[arg-type]
...
class LLaDAModelLM(PreTrainedModel):
    ...
    def forward(
        ...
        if not return_dict:
            output = (logits,) + (outputs.attn_key_values, hidden_states, attentions)
